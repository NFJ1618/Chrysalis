...
class Relation[T, R]:
    ...
    @property
    def invaraints(self) -> list[Callable[[R, R], bool]]:
...
class KnowledgeBase[T, R]:
    ...
    def register(
        ...
        if {transform_name, invariant.__name__} == {_LAMBDA_FUNCTION_NAME}:
            raise ValueError(
                "Registered transformation and invariant functions cannot be lambda functions."
            )
