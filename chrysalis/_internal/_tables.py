...
CREATE TABLE applied_transformation (
    id TEXT PRIMARY KEY,
    transformation TEXT NOT NULL,
    previous_applied_transformation TEXT,
    link_index INT NOT NULL,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,

    FOREIGN KEY (transformation) REFERENCES transformation(id),
    FOREIGN KEY (previous_applied_transformation) REFERENCES applied_transformation(id)
);
"""
...
SELECT * FROM sqlite_scan(?, ?) WHERE link_index = ?;
                    """,
            (str(sqlite_db), "applied_transformation", i),
        )
